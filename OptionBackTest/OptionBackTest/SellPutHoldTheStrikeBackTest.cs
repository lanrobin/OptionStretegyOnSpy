...
{
    public class SellPutHoldTheStrikeBackTest : BaseBackTestService
    {
        ...
        private ILogger<SellPutHoldTheStrikeBackTest> _logger;
        public SellPutHoldTheStrikeBackTest(ILogger<SellPutHoldTheStrikeBackTest> logger, Settings s) : base(s)
        {
            _logger = logger;
            SELL_PUT_PERCENT = 1.0 + MIU - NUM_OF_DELTA * DELTA;
        }
        ...
        public override void Calculate(DataCollection dc)
        {
            ...
            for (int i = 1; i < wv.Count; i++)
            {
                ...
                // we will keep the strike price until the close price higher then it.
                holdTheStrikePrice = Math.Max(holdTheStrikePrice, preClose);

                if(holdTheStrikePrice > preClose)
                {
                    _logger.LogWarning($"HOLD>>>>> holdTheStrikePrice:{holdTheStrikePrice}, preClose:{preClose}");
                }

                int sellPutStockPrice = (int)Math.Floor(holdTheStrikePrice * SELL_PUT_PERCENT);
            ...
            WriteToCVS($"SellPutHoldTheStrikeBackTest", result);
        }
    }
}
